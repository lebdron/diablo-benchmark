...
func decodeTransferTransaction(src io.Reader, manager nonceManager, provider parameterProvider) (*transferTransaction, error) {
	...
	to := common.BytesToAddress(tobuf)

	return newTransferTransaction(nonce, amount, from,
		&to, manager, provider), nil
}
...
func decodeInvokeTransaction(src io.Reader, manager nonceManager, provider parameterProvider) (*invokeTransaction, error) {
	...
	to := common.BytesToAddress(appidbuf)
...
type cachedParameterProvider struct {
	params   *parameters
	lock     sync.RWMutex
	provider *directParameterProvider
}

func newCachedParameterProvider(params *parameters, provider *directParameterProvider) *cachedParameterProvider {
...
func makeCachedParameterProvider(client ethclient.Client) (*cachedParameterProvider, error) {
	p := newDirectParameterProvider(client)
	params, err := p.getParams()
	if err != nil {
		return nil, err
	}

	return newCachedParameterProvider(params, p), nil
}
...
func (this *directParameterProvider) getParams() (*parameters, error) {
	...
	baseFee, err := this.client.EstimateBaseFee(ctx)
	if err != nil {
		return nil, err
	}

	params.maxPriorityFeePerGas, err = this.client.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}

	params.maxFeePerGas = baseFee.Add(baseFee, params.maxPriorityFeePerGas)
