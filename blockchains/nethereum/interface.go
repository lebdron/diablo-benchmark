...
func (this *BlockchainInterface) Client(params map[string]string, env, view []string, logger core.Logger) (core.BlockchainClient, error) {
	...
	confirmer = newPollblkTransactionConfirmer(logger, client, ctx, observerProvider)

	for key, value = range params {
		if key == "prepare" {
			logger.Tracef("use prepare method '%s'", value)
			provider, preparer, err =
				parsePrepare(value, logger, client, observerProvider)
			if err != nil {
				return nil, err
			}
			continue
...
func parsePrepare(value string, logger core.Logger, client *ethclient.Client, observerProvider *observerParameterProvider) (parameterProvider, transactionPreparer, error) {
	var preparer transactionPreparer
	var provider parameterProvider
	var err error
