...
func (this *BlockchainInterface) Client(params map[string]string, env, view []string, logger core.Logger) (core.BlockchainClient, error) {
	...
	for key, value := range params {
		if key == "prepare" {
			...
			provider, err = parsePrepare(value, logger, client, providerChan, ctx)
			if err != nil {
				return nil, err
			}
			continue
	...
	if provider == nil {
		...
		provider, err = parsePrepare("cached", logger, client, providerChan, ctx)
		if err != nil {
			return nil, err
		}
	}
...
func parsePrepare(
	...
	if value == "cached" {
		provider, err := newCachedParameterProvider(logger, blocks)
		if err != nil {
			return nil, err
		}
